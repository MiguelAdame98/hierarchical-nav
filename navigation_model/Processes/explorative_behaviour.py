...
#====================== Get Methods ===================================================================
    def define_is_agent_exploring(self, info_gain_coeff:float, policy_G:float, threshold:int=1)-> bool:
        ...
        """
        print("why no explore?",abs(info_gain_coeff),(threshold / 800) ,abs(info_gain_coeff) < (threshold / 800),policy_G < (threshold/ 8),policy_G ,(threshold/ 8)  )
        #if (abs(info_gain_coeff) < (threshold / 1000) and policy_G < (threshold/ 10)) :
        if (abs(info_gain_coeff) < (threshold / 800) and policy_G < (threshold/ 8)) :
